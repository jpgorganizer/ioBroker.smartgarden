 ...
 * Version:             0.1 ($Date: 2020-02-28 22:51:23 +0100 (Fr, 28 Feb 2020) $)
 * SVN:                 $Rev: 1922 $
					...
					case 'duration_value': {
						...
						request(options_power_socket_control, function(err, response, body){
							...
							if(err || !response) {
								...
								adapter.log.info('Command failure.');
							} else {
								// successful
								adapter.log.info('Command: successful response.statusCode/Message=' + response.statusCode + '/' + response.statusMessage);
							}
						})				
						...
						for (let prop2 in m[propattributes]) {
							for (let prop3 in m[propattributes][prop2]) {
								...
								if (a === '.activity_value') {
									...
									if (m.attributes[prop2][prop3] === 'SCHEDULED_ON' ||
									    m.attributes[prop2][prop3] === 'TIME_LIMITED_ON') {
 ...
 * 
 * @param   {string}  mainrev  Revision von Main im Format $Rev: 1922 $
 */
