 ...
 * SVN:                 $Rev: 2250 $ ($Date: 2020-08-30 12:25:08 +0200 (So, 30 Aug 2020) $)
 * contains some functions available at forum.iobroker.net, see function header
 */
"use strict";

// some variables 
const apirev = '$Rev: 2250 $';
...
function setGardenaLocationNotExists(lid) {
	if (LOCATIONS.hasOwnProperty('LOCATION_' + lid) === false) {
		ju.adapterloginfo(1, 'setGardenaLocationNotExists: new location added: LOCATION_' + lid);
...
function setGardenaDeviceNotExists(lid, did) {
	...
	if (LOCATIONS['LOCATION_' + lid].hasOwnProperty('DEVICE_' + did) === false) {
		ju.adapterloginfo(1, 'setGardenaDeviceNotExists: new device added: LOCATION_' + lid + '.DEVICE_' + did);
...
function setGardenaServiceNotExists(lid, did, sid, stype) {
	...
	if (LOCATIONS['LOCATION_' + lid]['DEVICE_' + did].hasOwnProperty('SERVICE_' + stype + '_' + sid) === false) {
		ju.adapterloginfo(1, 'setGardenaServiceNotExists: new service added: LOCATION_' + lid + '.DEVICE_' + did + '.SERVICE_' + stype + '_' + sid);
			...
			if (err || !response) {
				...
				exports.get_websocket(function(err, websocket) {
					if (err) {
						...
						ju.adapterloginfo(1, 'Refresh: get_websocket ... websocket=' + ju.makePrintable(websocket.substr(0,truncate_long_text_at_pos)) + ' trunc at ' + truncate_long_text_at_pos + ' chars');
						sgSetState('info.connection', true);
					}
				});
			}
...
function setValve(name, a, type, propval) {
	...
	if (configUseIrrigationAllowedWhileMowing === true) {
		...
		if (a === '.activity_value') {
			...
			setGardenaAttributeNotExists(nx1, val_i);
 ...
 * 
 * @param   {string}  mainrev  Revision von Main im Format $Rev: 2250 $
 */
