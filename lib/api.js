 ...
 * SVN:                 $Rev: 2757 $ ($Date: 2022-05-02 21:18:05 +0200 (Mo, 02 Mai 2022) $)
 * contains some functions available at forum.iobroker.net, see function header
 */
"use strict";

// some variables  
const apirev = '$Rev: 2757 $';
...
function putRequest (service_log, serviceid, service_command, service_control, value) {
	...
	request(options, function(err, response, body) {
		...
		if (err || !response || response.statusCode >= 300) {
			// failure
			ju.adapterloginfo(1, service_log + ' command failure.');
			ju.adapterloginfo(1, 'command: options=' + JSON.stringify(options));
			if (err) {
				e = err;
			...
			// successful
			ju.adapterloginfo(1, service_log + ' command successful. response.statusCode/Message=' + response.statusCode + '/' + response.statusMessage);
		}
	})
}
...
class Client {
	...
	on_close(close) {
		...
		{
			t = 'on_close: try reconnectWithRefreshToken'
			ju.consolelog(1, '### ' + ju.curTime() + " " + t);
			ju.adapterloginfo(1, t);
 ...
 * 
 * @param   {string}  mainrev  Revision von Main im Format $Rev: 2757 $
 */
